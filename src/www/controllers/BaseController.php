...
class BaseController extends \yii\web\Controller {
	...
	public function checkResponse($response, $model, $success = null, $fail = null)
	{
		...
		if ($response->isOk) {
			Yii::$app->session->setFlash('success', $response['message'] ? $response['message'] : 'Operação realizada com sucesso');
			return call_user_func($success, $response, $model, $this);
		} else {
			Yii::$app->session->setFlash('danger', $response['message'] ? $response['message'] : 'Não foi possível realizar a operação');
